...
class SelectFields
{
    ...
    public static function getSelectableFieldsAndRelations(
        ...
        $standardSelect = !isset($parentType->config['standardSelect']) ? true : $parentType->config['standardSelect'];

        if ($topLevel) {
            return [$select, $with];
        } else {
            return function ($query) use ($with, $select, $customQuery, $requestedFields, $ctx, $fieldObject, $standardSelect) {
                if ($customQuery) {
            ...
            if ($canSelect === true) {
                ...
                elseif (is_array($field['fields']) && $queryable) {
                    if (isset($parentType->config['model'])) {
                        ...
                        $with[$relationsKey] = self::getSelectableFieldsAndRelations($queryArgs, $field, $newParentType, $customQuery, false, $ctx, $fieldObject);
                    } else {
